...
class GeminiRAGIngestion(BaseRAGIngestion):
    ...
    async def store(
        ...
        # Get API credentials
        api_key = (
            cast(Optional[str], vector_store_config.get("api_key")) or GeminiModelInfo.get_api_key()
        )
        api_base = (
    ...
    async def _initiate_resumable_upload(
        ...
        custom_metadata = cast(
            Optional[List[Dict[str, Any]]], self.vector_store_config.get("custom_metadata")
        )
        if custom_metadata:

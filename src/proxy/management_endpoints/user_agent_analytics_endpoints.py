...
async def get_per_user_analytics(
    ...
    try:
        ...
        tag_records = await prisma_client.db.litellm_dailytagspend.find_many(where=where_clause)
            ...
            if record.api_key in api_key_to_user_id:
                ...
                if user_id not in user_metrics:
                    user_metrics[user_id] = PerUserMetrics(
                        user_id=user_id, user_email=user_id_to_email.get(user_id), user_agent=tag
                    )
                else:
        ...
        results = sorted(
            list(user_metrics.values()), key=lambda x: x.successful_requests, reverse=True
        )

    ...
    if output_format == "json":
        ...
        for col_name in requested_columns:
            if col_name in column_configs:
                ...
                table.add_column(
                    config["header"], style=config["style"], justify=config.get("justify", "left")
                )
            else:
...
def _filter_model(model, model_regex, access_group_regex):
    ...
    if access_group_regex:
        if not isinstance(access_groups, list):
            return False
        if not any(
            isinstance(group, str) and access_group_regex.search(group) for group in access_groups
        ):
            return False
    return True
...
def _get_filtered_model_list(
    ...
    access_group_regex = (
        re.compile(only_access_groups_matching_regex) if only_access_groups_matching_regex else None
    )
...
@click.option(
    "--dry-run", is_flag=True, help="Show what would be imported without making any changes."
)
@click.option(

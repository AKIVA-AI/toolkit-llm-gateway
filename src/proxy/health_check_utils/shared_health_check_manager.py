...
class SharedHealthCheckManager:
    ...
    async def cache_health_check_results(
        self, healthy_endpoints: List[Dict[str, Any]], unhealthy_endpoints: List[Dict[str, Any]]
    ) -> None:
        """
    ...
    async def perform_shared_health_check(
        ...
        if lock_acquired:
            try:
                ...
                verbose_proxy_logger.info(
                    "Pod %s running health check for %d models", self.pod_id, len(model_list)
                )
            ...
            verbose_proxy_logger.warning(
                "Pod %s falling back to local health check (no cache available)", self.pod_id
            )

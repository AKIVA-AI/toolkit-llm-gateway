...
class GeminiVideoConfig(BaseVideoConfig):
    ...
    def map_openai_params(
        ...
        openai_params_to_map = {
            param for param in supported_openai_params if param not in {"model", "prompt"}
        }
    ...
    def get_complete_url(
        ...
        if api_base is None:
            api_base = (
                get_secret_str("GEMINI_API_BASE") or "https://generativelanguage.googleapis.com"
            )
    ...
    def transform_video_status_retrieve_response(
        ...
        video_obj = VideoObject(
            id=video_id, object="video", status="processing" if not is_done else "completed"
        )
        return video_obj

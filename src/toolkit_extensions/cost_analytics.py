    ...
    def get_total_cost(
        ...
        with get_session() as session:
            ...
            query = self._apply_filters(
                query, session, start_date, end_date,
                user_email, team_name, project_name, model
            )
    ...
    def get_cost_by_model(
        ...
        with get_session() as session:
            query = session.query(
                ...
                func.sum(LLMRequest.prompt_tokens + LLMRequest.completion_tokens).label("total_tokens"),
            ).group_by(LLMRequest.model)
            
            # Apply filters
            query = self._apply_filters(
                query, session, start_date, end_date,
                user_email, team_name, project_name, None
            )
    ...
    def get_cost_by_user(
        ...
        with get_session() as session:
            query = session.query(
                ...
                User, LLMRequest.user_id == User.id
            )
            
            # Apply filters BEFORE group by and limit
            query = self._apply_filters(
                query, session, start_date, end_date,
                None, team_name, None, None
            )
            
            # Then group by, order, and limit
            query = query.group_by(User.email).order_by(
                func.sum(LLMRequest.total_cost).desc()
            ).limit(limit)
    ...
    def get_cost_by_team(
        ...
        with get_session() as session:
            query = session.query(
                ...
                Team, LLMRequest.team_id == Team.id
            )
            ...
            # Then group by, order, and limit
            query = query.group_by(Team.name).order_by(
                func.sum(LLMRequest.total_cost).desc()
            ).limit(limit)
    ...
    def get_cost_by_project(
        ...
        with get_session() as session:
            query = session.query(
                ...
                Project, LLMRequest.project_id == Project.id
            )
            ...
            # Then group by, order, and limit
            query = query.group_by(Project.name).order_by(
                func.sum(LLMRequest.total_cost).desc()
            ).limit(limit)
    ...
    def get_performance_stats(
        ...
        with get_session() as session:
            ...
            cache_hit_sum = func.sum(
                case((LLMRequest.cache_hit == True, 1), else_=0)
            )
            error_sum = func.sum(
                case((LLMRequest.error_message.isnot(None), 1), else_=0)
            )
            ...
            cache_hit_rate = (result.cache_hits / result.total_requests * 100) if result.cache_hits else 0.0
    ...
    def get_summary(
        ...
        return {
            ...
            "by_user": self.get_cost_by_user(
                start_date, end_date, team_name, limit=10
            )[:10] if not user_email else [],
            "by_team": self.get_cost_by_team(
                start_date, end_date, limit=10
            )[:10] if not team_name else [],
            "by_project": self.get_cost_by_project(
                start_date, end_date, team_name, limit=10
            )[:10] if not project_name else [],

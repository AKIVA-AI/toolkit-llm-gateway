...
class ParallelAISearchRequest(_ParallelAISearchRequestRequired, total=False):
    ...
    source_policy: (
        _ParallelAISourcePolicy  # Optional - source policy for allowed/disallowed domains
    )
...
class ParallelAISearchConfig(BaseSearchConfig):
    ...
    def validate_environment(
        ...
        **kwargs,
        ...
        """
        api_key = (
            api_key or get_secret_str("PARALLEL_AI_API_KEY") or get_secret_str("PARALLEL_API_KEY")
        )
        if not api_key:
            raise ValueError(
